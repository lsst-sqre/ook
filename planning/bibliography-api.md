...
**D1 — Resource IDs become time-ordered, and existing IDs are re-minted.** *(Q1; issue 2.3)* IDs switch to a Snowflake-style layout — a millisecond timestamp in the high bits plus random low bits — inside the existing 60-bit / 12-character Crockford Base32 envelope, so the API format is unchanged and keyset ordering becomes creation order. Concrete layout: 43 bits of milliseconds since a custom epoch of 2025-01-01T00:00:00Z (~278 years of runway), then 17 random low bits (~131k values per millisecond; the `ON CONFLICT DO NOTHING` + retry from D3 backstops any same-millisecond collision). UUIDv7 was considered and rejected as the basis: a full UUIDv7 is 128 bits and would break the envelope, the `BIGINT` column type, and the API format, while a 60-bit truncation keeps its 1970-anchored 48-bit timestamp and leaves only 12 random bits — a strictly worse split that is no longer a standard UUID anyway. The generator moves into the storage layer (inside the ingest transaction). A one-time migration re-mints existing rows in `date_created` order — deriving timestamps from `date_created` and keeping IDs strictly increasing when rows share a millisecond; this is acceptable now because no external consumer holds resource IDs yet, and it becomes strictly harder after the Agent Search sync and the links FK land.
