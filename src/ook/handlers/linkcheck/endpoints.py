...
@router.post(
    ...
    description=(
        ...
        " ``broken`` with no ``status_code`` at all (Ook received no"
        " response — a connection, TLS, or DNS failure at its egress). A"
        " ``broken`` URL that did answer with a status code is Ook's own"
        " evidence and is not eligible, and neither is any other status."
