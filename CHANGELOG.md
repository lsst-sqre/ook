...
- `POST /ook/linkcheck/checks/{id}/contributions` now also accepts results for a member URL Ook recorded `broken` without ever getting a terminal HTTP status out of the origin — nothing answered (a connection, TLS, DNS, or timeout failure at Ook's own egress), or responses came back but the redirect chain never resolved to a final status. Either way the URL shows as `broken` with a null `status_code`, and that verdict rests on evidence Ook never obtained, which is the same "could not verify from its own vantage point" case bot-blocked URLs are eligible for. A client that did reach the origin can settle it: a contributed 2xx resolves the URL to `ok` (or `redirected`) with its failing streak cleared and the contributing repository recorded, and a contributed failure simply extends the streak. A `broken` URL that did answer with a status code remains ineligible, because that verdict is Ook's own evidence; so do `pending`, `ok`, `redirected`, `failing`, and `unsupported`. The rejection reason stays `not_blocked` for wire compatibility, and its `message` now names the URL's actual status and the status code behind it, if any.
...
- Every datetime-valued field in the link-check API now carries the `date_` prefix the rest of Ook's API already uses: `checked_at` is now `date_checked`, `last_checked_at` is `date_last_checked`, `last_ok_at` is `date_last_ok`, `failing_since` is `date_failing_since`, and `next_check_at` is `date_next_check`. This changes the check-status response (`GET /ook/linkcheck/checks/{id}`), the URL and origin-link read endpoints, and the `results[].date_checked` field of the new contributions request body. Documenteer's link-check client is the only known consumer, and the release carrying it is still unpublished. Migration `f43554a10acb` renames the underlying `checked_url` and `linkcheck_contribution` columns in place — along with the two indexes whose names embed them — so stored history is preserved.

- `OOK_OIDC_AUDIENCE` is a new required setting with no default, so Ook will not start until it is set. It is the audience GitHub Actions OIDC id-tokens must be minted for; set it to the deployment's public base URL so a token minted for one Ook environment is not replayable against another. Every Phalanx environment must set it before this release is deployed.

### New features

- Added `POST /ook/linkcheck/checks/{id}/contributions`, which lets a client that re-checked a link from its own vantage point contribute what it saw. Only URLs that are members of the check and currently `blocked` are eligible — the case Ook's own egress cannot resolve — and a batch applies entry by entry: ineligible entries come back in `rejected` with a per-URL reason (`not_a_member`, `not_blocked`, `unsupported_url`, `duplicate`), and the rest still apply. Accepted results run through the same status-transition engine, retry ladder, and `OOK_LINKCHECK_FRESHNESS_TTL` as Ook's own checks — a success resolves the URL to `ok` (or `redirected`, with its redirect metadata), a 404 advances the failing-to-broken ladder, and a 403, 429, or 503 is inconclusive and leaves it `blocked`. Each request must carry a GitHub Actions OIDC id-token minted for `OOK_OIDC_AUDIENCE`, whose `repository`, `run_id`, and `workflow_ref` claims are recorded as the contribution's provenance; the advisory `environment` block is constrained too, with `run_url`, `checker_version`, and `repository` length-bounded and `run_url` required to be an http(s) URL since it is only ever rendered as a link. Like the check submission endpoint, it is write-protected by Gafaelfawr at the ingress and shares its `OOK_LINKCHECK_MAX_URLS_PER_CHECK` cap, rejecting an oversized batch whole with a `too_many_urls` error.

- Link-check results now record where they were observed from. The per-URL entries of a check's poll response and the URL read endpoint (`GET /ook/linkcheck/urls`) carry `result_source` (`server` or `contribution`) and `contributed_by`, so a report can render "externally verified by <repository> CI" for a URL that Ook's own egress is bot-blocked from. Ook's own checks report `server` with no repository, and a later server check takes a contributed URL's state back over.

- Added the GitHub Actions OIDC verifier service behind those contributions: it caches GitHub's JWKS and verifies a presented id-token's signature, issuer, audience, and expiry. Time-based claims are judged with 30 seconds of clock-skew leeway, so Ook's clock lagging GitHub's minting clock does not reject a genuine token as not yet valid. A token whose key ID is not in the cached set triggers an off-schedule refetch, so a key rotation mid-TTL does not reject good tokens; that refetch is rate-limited to once a minute, so tokens naming invented key IDs cannot amplify into a request to GitHub apiece, and a failed refetch keeps verifying against the cached keys. The verifier is a process-wide singleton on the shared HTTP client, so its key cache applies across requests. A valid token from any repository is accepted: OIDC attests provenance here, while authorization remains the Gafaelfawr scope at the ingress.

### Other changes

- Added the `linkcheck_contribution` table, which records each contributed per-URL result alongside the provenance it arrived with. Contribution rows are retained with their check and purged with it. This release requires two Alembic migrations on top of `6f68334c9c9b`: `0bfe17a5f990` adds that table and the `result_source` and `contributed_by` columns on `checked_url`, where existing results backfill to the `server` source, and `f43554a10acb` renames link-check's datetime columns.
...
- Documented Ook's `date_` prefix convention for datetime and date-valued API fields in the development guide's style guide, covering both JSON field names (`date_created`, `date_checked`) and the capitalized form Ook's own response headers use (`X-Ook-Inventory-Date-Fetched`), along with its one exception: a field carrying a verbatim HTTP header value, such as the intersphinx domain's `last_modified`, mirrors the wire name instead. A new introspection test enforces the field-name half against every Pydantic model under `src/ook/handlers`, resolving `Annotated[...]` wrappers and optional unions, and fails naming the offending model and field; needing no database or app fixture, it runs in the `test-unit` session.
